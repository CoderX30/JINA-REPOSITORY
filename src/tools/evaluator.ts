...
function getRejectAllAnswersPrompt(question: string, answer: AnswerAction, allKnowledge: KnowledgeItem[]): PromptPair {
  logDebug(`📝 [${TOOL_NAME}] Generating reject-all-answers prompt:`, { 
    question: question.length <= 50 ? question : `${question.length} chars`,
    answer: (answer.answer?.length || 0) <= 50 ? answer.answer : `${answer.answer?.length || 0} chars`,
    knowledgeItemsCount: allKnowledge.length,
    referencesCount: answer.references?.length || 0
  });
  
  const KnowledgeStr = getKnowledgeStr(allKnowledge);
  logDebug(`📚 [${TOOL_NAME}] Knowledge string prepared:`, { 
    knowledgeStringCount: KnowledgeStr.length,
    totalKnowledgeLength: KnowledgeStr.join('').length
  });
...
`
  ...
  logDebug(`📝 [${TOOL_NAME}] Reject-all-answers prompt generated:`, { 
    systemPrompt: prompt.system.length <= 50 ? prompt.system : `${prompt.system.length} chars`,
    userPrompt: prompt.user.length <= 50 ? prompt.user : `${prompt.user.length} chars`
  });
...
function getDefinitivePrompt(question: string, answer: string): PromptPair {
  logDebug(`📝 [${TOOL_NAME}] Generating definitive evaluation prompt:`, { 
    question: question.length <= 50 ? question : `${question.length} chars`,
    answer: answer.length <= 50 ? answer : `${answer.length} chars`
  });
...
function getFreshnessPrompt(question: string, answer: AnswerAction, currentTime: string): PromptPair {
  logDebug(`📝 [${TOOL_NAME}] Generating freshness evaluation prompt:`, { 
    question: question.length <= 50 ? question : `${question.length} chars`,
    answer: (answer.answer?.length || 0) <= 50 ? answer.answer : `${answer.answer?.length || 0} chars`,
...
`
  ...
  logDebug(`📝 [${TOOL_NAME}] Freshness prompt generated:`, { 
    systemPrompt: prompt.system.length <= 50 ? prompt.system : `${prompt.system.length} chars`,
    userPrompt: prompt.user.length <= 50 ? prompt.user : `${prompt.user.length} chars`,
    answerJson: JSON.stringify(answer).length <= 50 ? JSON.stringify(answer) : `${JSON.stringify(answer).length} chars`
  });
...
function getCompletenessPrompt(question: string, answer: string): PromptPair {
  logDebug(`📝 [${TOOL_NAME}] Generating completeness evaluation prompt:`, { 
    question: question.length <= 50 ? question : `${question.length} chars`,
    answer: answer.length <= 50 ? answer : `${answer.length} chars`
  });
...
`
  ...
  logDebug(`📝 [${TOOL_NAME}] Completeness prompt generated:`, { 
    systemPrompt: prompt.system.length <= 50 ? prompt.system : `${prompt.system.length} chars`,
    userPrompt: prompt.user.length <= 50 ? prompt.user : `${prompt.user.length} chars`
  });
...
function getPluralityPrompt(question: string, answer: string): PromptPair {
  logDebug(`📝 [${TOOL_NAME}] Generating plurality evaluation prompt:`, { 
    question: question.length <= 50 ? question : `${question.length} chars`,
    answer: answer.length <= 50 ? answer : `${answer.length} chars`
  });
...
`
  ...
  logDebug(`📝 [${TOOL_NAME}] Plurality prompt generated:`, { 
    systemPrompt: prompt.system.length <= 50 ? prompt.system : `${prompt.system.length} chars`,
    userPrompt: prompt.user.length <= 50 ? prompt.user : `${prompt.user.length} chars`
  });
...
function getQuestionEvaluationPrompt(question: string): PromptPair {
  logDebug(`📝 [${TOOL_NAME}] Generating question evaluation prompt:`, { 
    question: question.length <= 50 ? question : `${question.length} chars`
  });
...
<think>`
  ...
  logDebug(`📝 [${TOOL_NAME}] Question evaluation prompt generated:`, { 
    systemPrompt: prompt.system.length <= 50 ? prompt.system : `${prompt.system.length} chars`,
    userPrompt: prompt.user.length <= 50 ? prompt.user : `${prompt.user.length} chars`
  });
...
export async function evaluateQuestion(
  ...
  logDebug(`🔍 [${TOOL_NAME}] ===== STARTING QUESTION EVALUATION =====`);
  logDebug(`🔍 [${TOOL_NAME}] Question evaluation parameters:`, { 
    question: question.length <= 50 ? question : `${question.length} chars`,
    hasTokenTracker: !!trackers.tokenTracker,
    hasActionTracker: !!trackers.actionTracker,
    schemaLanguage: schemaGen.languageCode
  });

  try {
    const generator = new ObjectGeneratorSafe(trackers.tokenTracker);
    logDebug(`🤖 [${TOOL_NAME}] ObjectGeneratorSafe initialized for question evaluation`);
    
    const prompt = getQuestionEvaluationPrompt(question);
    logDebug(`📝 [${TOOL_NAME}] Question evaluation prompt prepared`);

    logDebug(`🤖 [${TOOL_NAME}] Calling AI model for question evaluation`);
    ...
    logDebug(`🤖 [${TOOL_NAME}] AI model response received:`, { 
      thinking: result.object.think?.length <= 50 ? result.object.think : `${result.object.think?.length || 0} chars`,
    ...
    logDebug(`🔍 [${TOOL_NAME}] Question evaluation complete:`, { 
      evaluationTypesCount: types.length,
      evaluationTypes: types,
      aiReasoning: result.object.think?.length <= 50 ? result.object.think : `${result.object.think?.length || 0} chars`
    });
    ...
    logError(`❌ [${TOOL_NAME}] Error in question evaluation:`, { 
...
async function performEvaluation<T>(
  ...
  logDebug(`🔧 [${TOOL_NAME}] ===== STARTING ${evaluationType.toUpperCase()} EVALUATION =====`);
  logDebug(`🔧 [${TOOL_NAME}] Evaluation parameters:`, { 
    evaluationType,
    promptSystem: prompt.system.length <= 50 ? prompt.system : `${prompt.system.length} chars`,
    promptUser: prompt.user.length <= 50 ? prompt.user : `${prompt.user.length} chars`,
    hasTokenTracker: !!trackers.tokenTracker,
    hasActionTracker: !!trackers.actionTracker,
    schemaLanguage: schemaGen.languageCode
  });
  ...
  const generator = new ObjectGeneratorSafe(trackers.tokenTracker);
  logDebug(`🤖 [${TOOL_NAME}] ObjectGeneratorSafe initialized for ${evaluationType} evaluation`);
  
  logDebug(`🤖 [${TOOL_NAME}] Calling AI model for ${evaluationType} evaluation`);
  const result = await generator.generateObject({
    model: TOOL_NAME,
  ...
  logDebug(`🤖 [${TOOL_NAME}] ${evaluationType} evaluation response received:`, { 
    thinking: result.object.think?.length <= 50 ? result.object.think : `${result.object.think?.length || 0} chars`,
    evaluationResult: result.object.pass ? '✅ PASSED' : '❌ FAILED',
    evaluationType: result.object.type || evaluationType,
    usage: result.usage
  });
  ...
  logDebug(`🔧 [${TOOL_NAME}] ${evaluationType} evaluation complete:`, { 
...
export async function evaluateAnswer(
  ...
  logDebug(`🔍 [${TOOL_NAME}] ===== STARTING ANSWER EVALUATION =====`);
  logDebug(`🔍 [${TOOL_NAME}] Answer evaluation parameters:`, { 
    question: question.length <= 50 ? question : `${question.length} chars`,
    answer: (action.answer?.length || 0) <= 50 ? action.answer : `${action.answer?.length || 0} chars`,
    evaluationTypesCount: evaluationTypes.length,
  ...
  for (const evaluationType of evaluationTypes) {
    logDebug(`🎯 [${TOOL_NAME}] Processing evaluation type: ${evaluationType}`);
    ...
    switch (evaluationType) {
      ...
      case 'completeness':
        ...
        break;
      case 'strict':
        logDebug(`📝 [${TOOL_NAME}] Generating strict evaluation prompt with knowledge context`);
        prompt = getRejectAllAnswersPrompt(question, action, allKnowledge);
        break;
      default:
        logError(`❌ [${TOOL_NAME}] Unknown evaluation type encountered:`, { evaluationType });
    }
    
    if (prompt) {
      logDebug(`🔧 [${TOOL_NAME}] Prompt generated for ${evaluationType}, proceeding with evaluation`);
      try {
        ...
        logDebug(`✅ [${TOOL_NAME}] ${evaluationType} evaluation completed successfully:`, { 
          evaluationResult: (result?.object as EvaluationResponse)?.pass ? '✅ PASSED' : '❌ FAILED',
          tokensUsed: result?.usage.totalTokens
        });
      } catch (error) {
        logError(`❌ [${TOOL_NAME}] Error performing ${evaluationType} evaluation:`, { 
          error: error instanceof Error ? error.message : String(error),
          errorType: error instanceof Error ? error.constructor.name : 'Unknown',
          evaluationType
        });
        return { pass: false, think: `Error ${evaluationType} immediately return false, probably due to bad prompt?`, type: evaluationType } as EvaluationResponse
      }

      // fail one, return immediately
      if (!(result?.object as EvaluationResponse)?.pass) {
        logDebug(`❌ [${TOOL_NAME}] Evaluation failed for ${evaluationType}, returning immediately:`, { 
          failureReason: ((result?.object as EvaluationResponse)?.think?.length || 0) <= 50 ? (result?.object as EvaluationResponse)?.think : `${(result?.object as EvaluationResponse)?.think?.length || 0} chars`,
          evaluationType
        });
        return result?.object as EvaluationResponse;
      } else {
        logDebug(`✅ [${TOOL_NAME}] ${evaluationType} evaluation passed, continuing to next evaluation type`);
      }
    } else {
      logError(`❌ [${TOOL_NAME}] No prompt generated for evaluation type:`, { evaluationType });
    }
  }

  logDebug(`🎉 [${TOOL_NAME}] ===== ALL EVALUATIONS PASSED =====`);
  logDebug(`🎉 [${TOOL_NAME}] Final evaluation result:`, { 
    allEvaluationsPassed: true,
    totalEvaluationsRun: evaluationTypes.length,
    finalResult: (result?.object as EvaluationResponse)?.pass ? '✅ PASSED' : '❌ FAILED'
  });
